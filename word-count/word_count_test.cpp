...
TEST_CASE("multiple occurrences of a word",
          "[2a3091e5-952e-4099-9fac-8f85d9655c0e]") {
    const std::map<std::string, int> expected{
        {"one", 1}, {"fish", 4}, {"two", 1}, {"red", 1}, {"blue", 1}};
    REQUIRE(expected ==
            word_count::words("one fish two fish red fish blue fish"));
}
...
TEST_CASE("with quotations", "[be72af2b-8afe-4337-b151-b297202e4a7b]") {
    const std::map<std::string, int> expected{{"joe", 1},   {"can't", 1},
                                              {"tell", 1},  {"between", 1},
                                              {"large", 2}, {"and", 1}};
    REQUIRE(expected ==
            word_count::words("Joe can't tell between 'large' and large."));
}

TEST_CASE("substrings from the beginning",
          "[8d6815fe-8a51-4a65-96f9-2fb3f6dc6ed6]") {
    const std::map<std::string, int> expected{
        {"joe", 1}, {"can't", 1}, {"tell", 1}, {"between", 1},
        {"app", 1}, {"apple", 1}, {"and", 1},  {"a", 1}};
    REQUIRE(expected ==
            word_count::words("Joe can't tell between app, apple and a."));
}

TEST_CASE("multiple spaces not detected as a word",
          "[c5f4ef26-f3f7-4725-b314-855c04fb4c13]") {
    const std::map<std::string, int> expected{{"multiple", 1},
                                              {"whitespaces", 1}};
    REQUIRE(expected == word_count::words(" multiple   whitespaces"));
}

TEST_CASE("alternating word separators not detected as a word",
...
TEST_CASE("quotation for word with apostrophe",
