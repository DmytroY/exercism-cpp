...
TEST_CASE("encode single characters only are encoded without count",
          "[encode]") {
...
TEST_CASE("encode single characters mixed with repeated characters",
          "[encode]") {
    REQUIRE(run_length_encoding::encode(
...
TEST_CASE("decode single characters with repeated characters", "[decode]") {
    REQUIRE(run_length_encoding::decode("12WB12W3B24WB") ==
            "WWWWWWWWWWWWBWWWWWWWWWWWWBBBWWWWWWWWWWWWWWWWWWWWWWWWB");
}
